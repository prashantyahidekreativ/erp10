...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        model:  'account.tax',
        fields: ['name','amount', 'price_include', 'include_base_amount', 'amount_type','precision', 'children_tax_ids','sub_tax_ids'],
        domain: null,
        loaded: function(self, taxes){
            ...
            _.each(self.taxes_by_id, function(tax) {
                tax.sub_tax_ids = _.map(tax.sub_tax_ids, function (sub_tax_id) {
                    return self.taxes_by_id[sub_tax_id];
                });
            });
        },
    },{
        ...
        fields: ['display_name', 'list_price','price','pos_categ_id', 'taxes_id', 'barcode', 'default_code', 
                 ...
                 'product_tmpl_id','tracking','pos_taxes_ids'],
...
exports.Orderline = Backbone.Model.extend({
    ...
    compute_all: function(taxes, price_unit, quantity, currency_rounding) {
        ...
        _(taxes).each(function(tax) {
            ...
            if (tax.amount_type === 'group'){
            	...
            	if (tax.sub_tax_ids.length > 0){
            	    var ret = self._compute_all(tax.sub_tax_ids[0], base, quantity);	
	            	ret = round_di(ret, self.pos.dp[tax.precision[1]]);
            	...
            	var prec=self.pos.dp[tax.precision[1]];
            	tax_amount = round_di(tax_amount, prec);
    ...
    get_all_prices: function(){
        ...
        var pos_taxes = product.pos_taxes_ids;
        ...
        var order_fiscal_position = current_order && current_order.fiscal_position;
        if (order_fiscal_position){
             if (order_fiscal_position.tax_type == 'gst'){
        	      taxes_ids= pos_taxes
              }  
        }
        ...
        if (order_fiscal_position){
            if (order_fiscal_position.tax_type == 'gst' & order_fiscal_position.price_include){
                product_taxes = $.grep(product_taxes,function(tx){
        	           
        		       return tx.price_include
        	     });
           }
        }
