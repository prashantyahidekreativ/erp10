...
supported_currency = ['LVL','EUR','USD', 'UAH','INR','GBP']
...
#    if sum < 0 or sum > 999999999999.99 : 
    if sum > 999999999999.99 : 
        raise Exception('Sum out of bounds: must be less than 999999999999.99')
...
#--------------for currencies with 100 fractions
    ...
    if language == 'en_US' :
        if sum_cur == 1 or (str(sum_cur)[-1] == '1' and str(sum_cur)[-2] != '1'): # is the currency sum one
            if currency == 'LVL':
                ...
                cur_in_words += u' rupee'
            elif currency == 'GBP':
                cur_in_words += u' Pound'                
        else:
            if currency == 'LVL':
                cur_in_words += u' Latvian lats'
            elif currency == 'EUR':
                ...
                cur_in_words += u' rupees'
            elif currency == 'GBP':
                cur_in_words += u' pounds'                
        if sum_frc == 1 or (str(sum_frc)[-1] == '1' and str(sum_frc)[-2] != '1'): # is the fraction sum one
            if currency == 'LVL':
                ...
                frc_in_words += u' paisa'
            elif currency == 'GBP':
                cur_in_words += u' penny'                  
        else:
            if currency == 'LVL':
                ...
                frc_in_words += u' paise'
            elif currency == 'GBP':
                cur_in_words += u' pence'                  
    ...
    ret = (cur_in_words + ' ' + frc_in_words).strip().encode('utf-8')
    ret=ret.replace("zero paise","")
    return ( ret.title()+' Only.')
