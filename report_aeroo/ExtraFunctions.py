...
from odoo import registry, models, _
...
import string
from odoo.osv.orm import browse_record_list #TODO v8?
...
class ExtraFunctions(object):
    ...
    def __init__(self, cr, uid, report_id, context):
        ...
        self.functions = {
            ...
            'get_val_recs':self._get_val_recs,
    ...
    def _get_note(self,obj,number=0,context=None):
        if isinstance(obj, models.Model):
            if isinstance(obj, (str,unicode)):
                model = obj
            else:
                model = obj._name
            domain=[('model_id','=',model),('state','=',obj.state),('print_no','=',number)]
            if model == 'account.invoice':
               domain+=[('partner','=',self.type)] 
            note=obj.env['kts.report.template'].search(domain).note
            return note
        else:
            return ''   
    
    
    def _get_company_address(self,obj,context=None):
        if isinstance(obj, models.Model):
            com=self.context['user'].company_id
            address= (com.street if com.street else '')+' '+(com.street2 if com.street2 else '')+' '+(com.city if com.city else '' )+' '+(com.state_id.name if com.state_id else '' )+' '+(com.country_id.name if com.country_id else '' )
            return address
        else:
            return ''   
    
    def _get_company_name(self,obj,context=None):
        if isinstance(obj, models.Model):
            return self.context['user'].company_id.name
        else:
            return ''   
    def _get_val_recs(self,obj,context=None):
        if isinstance(obj, models.Model):
           if context and isinst_get_move_linesance(context, dict):
                new_context = obj._context.copy()
                new_context.update(context)
                return obj.with_context(new_context).get_val_recs()
           else:
                return obj.get_val_recs()
        return ''
    
    
    def _get_move_lines(self, obj,context=False):
        if isinstance(obj, models.Model):
           if context:
                return obj.get_move_lines(context)
           else:
                return obj.get_move_lines()
        return ''  
    ...
    def _get_report_xml(self):
        env = odoo.api.Environment(self.cr, self.uid, self.context or {})
        return env['ir.actions.report.xml'].browse(self.report_id)
    ...
    def _currency2text(self, currency):
        def c_to_text(sum, currency=currency, language=None):
            ...
            return unicode(supported_language.get(language or self._get_lang()).currency_to_text(sum, currency).title(), "UTF-8")
        return c_to_text
