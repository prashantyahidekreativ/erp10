...
class Aeroo_report(report_sxw):
    ...
    def __init__(self, cr, name, table, rml=False, parser=False, header=True, store=False):
        ...
        try:
            ...
            if report_xml_ids:
                if len(report_xml_ids.ids)> 1:
                   report_xml = report_xml_ids[0]
                else:
                    report_xml = report_xml_ids
            else:
                report_xml = False
    ...
    def getObjects_mod(self, cr, uid, ids, rep_type, context):
        if rep_type=='aeroo':
            ...
            table_obj =env[self.table]
            #table_obj = registry(cr.dbname).get(self.table)
            return table_obj.browse(ids) or []
    ...
    def get_styles_file(self, cr, uid, report_xml, company=None, context=None):
        pool = registry(cr.dbname)
        style_io=None
        if report_xml.styles_mode != 'default' and report_xml.styles_mode != False :
    ...
    def get_docs_conn(self, cr):
        ...
        env = odoo.api.Environment(cr, 1, {})
        icp = env['ir.config_parameter']
        docs_host = icp.get_param('aeroo.docs_host') or 'localhost'
        docs_port = icp.get_param('aeroo.docs_port') or '8989'
        docs_auth_type = icp.get_param('aeroo.docs_auth_type') or False
        docs_username = icp.get_param('aeroo.docs_username') or 'anonymous'
        docs_password = icp.get_param('aeroo.docs_password') or 'anonymous'
    ...
    def create_aeroo_report(self, cr, uid, ids, data, report_xml, context=None, output='odt'):
        ...
        model_id = env['ir.model'].search([('model','=',context.get('active_model', data['model']) or data['model'])]).id
   ...
   # def create_source_pdf(self, cr, uid, ids, data, report_xml, context=None): V-odoo-ERP9
