...
class report_aeroo_import(models.TransientModel):
    ...
    def install_report(self):
        ...
        if report_obj.search([('report_name','=',this.name)]):
            raise UserError(_('Report with service name "%s" already exist in system!') % this.name)
        fd = StringIO()
        fd.write(base64.decodestring(this.file))
        fd.seek(0)
        convert_xml_import(self.env.cr, 'report_aeroo', fd, {}, 'init', noupdate=True)
        fd.close()
        self.write({'state':'done'})
        report_id = report_obj.search([('report_name','=',this.name)])[-1]
        report = report_obj.browse(report_id)
        event_id = self.env['ir.values'].set_action(self.env.cr, self.env.uid, report.report_name, 'client_print_multi', report.model, 'ir.actions.report.xml,%d' % report_id)
        ...
        mod_id = mod_obj.search([('name', '=', 'action_aeroo_report_xml_tree')])[0]
    ...
    def next(self):
        ...
        if zipfile.is_zipfile(zip_stream):
            ...
            self.write({'name':rep_service,'info':info,'state':'info','file':base64.encodestring(data)})
        else:
            raise UserError(_('Is not Aeroo report file.'))

        mod_obj = self.env['ir.model.data']
        act_obj = self.env['ir.actions.act_window']

        mod_id = mod_obj.search([('name', '=', 'action_aeroo_report_import_wizard')])[0]
        res_id = mod_obj.read(mod_id, ['res_id'])['res_id']
        act_win = act_obj.read(res_id, [])
