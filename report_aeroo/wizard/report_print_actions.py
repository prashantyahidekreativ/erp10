...
class report_print_actions(models.TransientModel):
    ...
    ### ends Fields
    @api.onchange('out_format_id')
    def onchange_out_format(self):
        if not self.out_format_id:
            return {}
        out_format = self.env['report.mimetypes'].read(int(self.out_format_id.id), ['code'])
        return { 'value':
