...
from odoo.modules import module #add V9
import encodings
...
#             for mod_path in module.ad_paths: #add V9
...
#             self.register_report(report['report_name'], report['model'], report['report_rml'], parser)
    ...
    def _lookup_report(self, name):
        if 'report.' + name in interface.report_int._reports:
            ...
            self._cr.execute("SELECT id, active, report_type, parser_state, \
    ...
    def change_input_format(self):
        if self.in_format:
           out_format = self.env['report.mimetypes'].search([('code','=',self.in_format)])
           return {
    ...
    @api.multi
    @api.depends('report_type', 'tml_source', 'report_sxw', 'report_sxw_content_data')
    def _compute_report_sxw_content(self):
        ...
        aeroo_ids = self.search([('report_type','=','aeroo'),('id','in',self.ids)])
        orig_ids = list(set(self.ids).difference(aeroo_ids.ids))
    ...
    report_sxw_content = fields.Binary('SXW content', compute='_compute_report_sxw_content')
    active = fields.Boolean('Active', help='Disables the report if unchecked.', default=True)
    ...
    fallback_false = fields.Boolean('Disable Format Fallback', help='Raises error on format convertion failure. Prevents returning original report file type if no convertion is available.')
